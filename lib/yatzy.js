/*
 * yatzy
 * https://github.com/sodper/yatzy-kata
 *
 * Copyright (c) 2014 Per Flitig
 * Licensed under the MIT license.
 */

'use strict';

// reduces to sum
var sum = function (previous, current) {
  return previous + current;
};

// creates a value filter function
var equal =  function(value) {
  return function (x) {
    return x === value;
  };
};

// reduces to array of pairs
var getPairs = function(last, now, index, array) {
  if(array.filter(equal(now)).length === 2) {
      last.push(now);
  }
  return last;
};

// reduces to array of threes
var getThrees = function(last, now, index, array) {
  if(array.filter(equal(now)).length === 3) {
      last.push(now);
  }
  return last;
};

// reduces to array of fours
var getFours = function(last, now, index, array) {
  if(array.filter(equal(now)).length === 4) {
      last.push(now);
  }
  return last;
};


/*
  commands
*/

var dieCommand = function(die) {
  return function(roll) {
    var filtered = roll.filter(equal(die));
    return filtered.reduce(sum, 0);
  };
};

var chanceCommand = function(roll) {
  return roll.reduce(sum, 0);
};

var yatzyCommand = function(roll) {
  return (roll.every(equal(roll[0]))) ? 50 : 0;
};

var pairCommand = function(roll) {
  var result = roll.reduce(function(last, now, index, array) {
    var filtered = array.filter(equal(now));
    return (filtered.length === 2 && now * 2 > last.score) ? { die: now, score: now * 2 } : last;
  }, { die: 0, score: 0 });
  return result.score;
};

var twoPairsCommand = function(roll) {
  var pairs = roll.reduce(getPairs, []);

  return (pairs.length === 4) ? pairs.reduce(sum, 0) : 0;
};

var threeOfKindCommand = function(roll) {
  var threes = roll.reduce(getThrees, []);
  return threes.reduce(sum, 0);
};

var fourOfKindCommand = function(roll) {
  var threes = roll.reduce(getFours, []);
  return threes.reduce(sum, 0);
};

// command object
var commands = {
  'ones': dieCommand(1),
  'twos': dieCommand(2),
  'threes': dieCommand(3),
  'fours': dieCommand(4),
  'fives': dieCommand(5),
  'chance': chanceCommand,
  'yatzy': yatzyCommand,
  'pair': pairCommand,
  'two pairs': twoPairsCommand,
  'three of a kind': threeOfKindCommand,
  'four of a kind': fourOfKindCommand,
};

exports.score = function(roll, category) {
  return commands[category](roll);
};
